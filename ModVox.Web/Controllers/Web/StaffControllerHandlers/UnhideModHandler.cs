...
public sealed class UnhideModHandler
{
    private readonly IModRepository _modRepository;
    private readonly IAccountAuthorizationService _authorizationService;

    public UnhideModHandler(IModRepository modRepository, IAccountAuthorizationService authorizationService)
    {
    ...
    public async Task<IResult> HandleAsync(HttpContext httpContext, Guid modId, CancellationToken cancellationToken)
    {
        ...
        return Results.Ok(new UnhideModResponse(updated.Id, updated.ModerationStatus));
    }
}

public sealed record UnhideModResponse(Guid ModId, string ModerationStatus);
