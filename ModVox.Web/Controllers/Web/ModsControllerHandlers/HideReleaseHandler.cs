...
public sealed class HideReleaseHandler
{
    ...
    public async Task<IResult> HandleAsync(Guid releaseId, HttpContext httpContext, CancellationToken cancellationToken)
    {
        ...
        if (release.IsHidden)
            return Results.Ok(new HideReleaseResponse(release.Id, release.IsHidden));

        var updated = release with { IsHidden = true };
        await _releaseRepository.UpdateAsync(updated, cancellationToken);
        return Results.Ok(new HideReleaseResponse(updated.Id, updated.IsHidden));
    }
}

public sealed record HideReleaseResponse(Guid ReleaseId, bool IsHidden);
