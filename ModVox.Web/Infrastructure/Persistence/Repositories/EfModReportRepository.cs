...
public sealed class EfModReportRepository : IModReportRepository
{
    ...
    public async Task AddAsync(ModReportRecord report, CancellationToken cancellationToken)
    {
        _db.ModReports.Add(report);
        await _db.SaveChangesAsync(cancellationToken);
    }

    public async Task<IReadOnlyList<ModReportRecord>> ListOpenAsync(CancellationToken cancellationToken)
        => await _db.ModReports.AsNoTracking()
            .Where(r => r.Status == ModReportStatus.Open)
            .OrderByDescending(r => r.CreatedAt)
            .ToListAsync(cancellationToken);

    public async Task<ModReportRecord?> GetByIdAsync(Guid reportId, CancellationToken cancellationToken)
        => await _db.ModReports.AsNoTracking().FirstOrDefaultAsync(r => r.Id == reportId, cancellationToken);

    public async Task UpdateAsync(ModReportRecord report, CancellationToken cancellationToken)
    {
